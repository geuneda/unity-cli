...
- UI 생성은 현재 `TextMeshProUGUI` / `TMP_InputField` 기준
- TMP Essential Resources가 없으면 첫 TMP UI 생성 시 자동 import 시도
...
실제 UI 확인이 필요하면 Editor를 직접 연 상태에서 `ui text.create`, `ui inputfield.create`, `resource get ui/hierarchy`를 순서대로 호출해 `selectableType`, `textComponentType`, `isSelected` 값을 확인한다.
...
4. UI 생성 시 `resource get ui/hierarchy`로 TMP 기반 생성 결과를 확인
5. 필요시 `ui focus` / `ui blur`로 EventSystem selection 전환을 검증
6. 필요시 workflow 파일로 이벤트 기반 자동화
7. `verify-editor.sh`로 end-to-end 검증

## 주의사항

- `ui text.create`로 생성되는 텍스트는 `TextMeshProUGUI`다.
- `ui inputfield.create`로 생성되는 입력 필드는 `TMP_InputField`다.
- `resource get ui/hierarchy`의 `inputField.textComponentType`로 TMP 적용 여부를 빠르게 확인할 수 있다.
- Unity EditMode에서는 입력 필드의 `isFocused`가 즉시 `true`로 보이지 않을 수 있다. 포커스 검증은 `isSelected`와 `editor/state.eventSystemSelectedObjectName`도 같이 본다.
- 기존 씬에 남아 있는 레거시 `Text` / `InputField`는 조회와 텍스트 변경 명령에서 호환 처리된다.

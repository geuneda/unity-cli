...
- `ui toggle.set|slider.set|scrollrect.set|inputfield.set-text`
...
기본 실행은 `core`, `ui-input`, `tests`, `editor-lifecycle` 네 stage를 모두 돌리고, `--stage`를 여러 번 주면 필요한 stage만 선택 실행할 수 있습니다.
...
- `ui-input` stage에서 현재 실제 검증하는 UI 상태:
  ...
  - Toggle `isOn` 변경
  - Slider `value` 변경
  - ScrollRect `normalizedPosition` 변경
  - InputField 텍스트 입력
